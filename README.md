...
| Platform | State |
|---|---|
| macOS arm64 (`maca64`) | Builds, loads, 81/81 tests pass. Verified locally against Homebrew ITK; the no-ITK-installed CI claim below is pending re-verification against the Phase 1 opcodes (see BUILDING.md). |
| Linux x86_64 (`glnxa64`) | Builds, loads. Last CI-verified at 34/34, before the Phase 1 opcodes; re-verification against the current 81-test suite on a runner with **no ITK installed** is pending. Must be built with GCC 12 or older; see BUILDING.md. |
